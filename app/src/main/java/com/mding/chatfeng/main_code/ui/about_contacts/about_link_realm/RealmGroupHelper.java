...
//  添加好友信息
    public void addRealmGroup(final CusDataGroup linkFriend) {
        linkFriend.setTotalId(linkFriend.getGroupId() + SplitWeb.getUserId());
    ...
    public void deleteRealmFriend(String friendId) {
        CusDataGroup dog = mRealm.where(CusDataGroup.class).equalTo(FILE_NAME, friendId+SplitWeb.getUserId()).findFirst();
        if (dog!=null) {
            mRealm.beginTransaction();
            dog.deleteFromRealm();
            mRealm.commitTransaction();
    ...
    public void updateHeadPath(String friendId, String img, String time) {
        CusDataGroup realmMsg = mRealm.where(CusDataGroup.class).equalTo(FILE_NAME, friendId+SplitWeb.getUserId()).findFirst();
        if (realmMsg!=null) {
            mRealm.beginTransaction();
    ...
    public void updateAll(String friendId, CusDataGroup cusDataFriendRelation) {
        ...
        if (realmMsg!=null) {
            ...
            realmMsg.setTotalId(cusDataFriendRelation.getGroupId() + SplitWeb.getUserId());
            mRealm.insertOrUpdate(realmMsg);
            mRealm.commitTransaction();
        }
    }
