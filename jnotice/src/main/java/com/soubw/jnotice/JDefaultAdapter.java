...
public class JDefaultAdapter extends JNoticeAdapter<JNoticeBean> {
    ...
    protected View getView(final int position, View convertView, ViewGroup parent, final JNoticeBean bean) {
        ...
        if (convertView != null) {
            ...
            holder.ivHeader = (ImageView) convertView.findViewById(R.id.ivHeader);
        ...
        if (bean.getCount() != 0){
            holder.tvTitle.setText("您收到"+bean.getCount()+"条新消息");
        }else if (trueBeans.size() > 3){
            holder.tvTitle.setText("您当前收到"+trueBeans.size()+"条通知，快去查看吧");
        }else{
            holder.tvTitle.setText("收到来自"+bean.getName()+"的消息");
            holder.tvContent.setVisibility(View.VISIBLE);
            holder.tvContent.setText(bean.getText());
        }
        holder.ivHeader.setImageResource(bean.getImg());
